...
def assign_labels(acts, audio_files, encoder):
    ...
    class_name = Path(acts['file_path']).parent.name
...
def visualize_cca_score(cca_coefs, save_path):
    ...
    plt.plot(range(1, len(mean_coefs) + 1), mean_coefs, label="Average CCA")
    plt.title("Average CCA Similarity Across Classes")
...
def save_cca(cca, layers, save_path):
    ...
    with open(save_path, 'w') as f:
        all_coefs = np.array(list(cca.values()))
        mean_coefs = np.mean(all_coefs, axis=0)
        
        f.write("Average CCA Across Classes\n")
        for i in range(len(mean_coefs)):
            f.write(f"{layers[i]}: {mean_coefs[i]}\n")
...
def main():
    ...
    for audio_path in tqdm(audio_files, desc="Activations computing process"):
    ...
    print(f"Results saved to {args.visual_save_path} and {args.text_save_path}")
