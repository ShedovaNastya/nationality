...
class EmbeddingsDataset(Dataset):
    ...
    def __init__(
            ...
            source_type,
            label_encoder,
            collection_name="language_embeddings"):
...
class LanguageCls(nn.Module):
    """
    Baseline model class for languages classification
    """
...
def train(model, train_loader, optimizer, criterion, num_epoch, device, test_loader):
    """
...
def get_loaders(source_path, source_type):
    ...
    train_dataset = EmbeddingsDataset(
        source_path, split="train", source_type=source_type, label_encoder=label_encoder)
    test_dataset = EmbeddingsDataset(
        source_path, split="test", source_type=source_type, label_encoder=label_encoder)
...
def main():
    ...
    parser.add_argument(
        "--eval_path",
        type=str,
        default="./scores/language.txt",
        help="Save path for evaluation results file (txt)"
    )
    parser.add_argument(
        "--visual_path",
        type=str,
        default="./result/language.png",
    ...
    model = LanguageCls(input_dim, 4).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0001)

    train(model, train_loader, optimizer,
          criterion, num_epoch=50, device=device, test_loader=test_loader)
    
    metrics = evaluate(model, test_loader, device)
    save_metrics(metrics, args.eval_path)
    save_visualization(
        model, test_dataset.embeddings.numpy(),
        label_encoder.inverse_transform(test_dataset.labels.numpy()), 
