...
def save_visualization(model, vectors, labels, save_path, device):
    ...
    label_map = {0: "neutral", 1: "sad", 2: "positive", 3: "angry", 4: "other"}
    ...
    plt.title("Visualization of emotion embeddings")
    plt.legend()
...
def main():
    ...
    parser.add_argument(
        "--source_path",
        type=str,
        default="./embeddings/numpy_embs.npy",
        help="Path to npy file or to chromadb collection folder"
    )
    parser.add_argument(
        "--eval_path",
        type=str,
        default="./results/metrics.txt",
        help="Save path for evaluation results file (txt)"
    )
    parser.add_argument(
        "--visual_path",
        type=str,
        default="./results/emotion_visualisation.png",
    ...
    model = EmotionCls(input_dim, 5).to(device)
